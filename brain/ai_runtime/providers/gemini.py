"""
Google Gemini Provider Adapter for DevBuddy 2.0 AI Runtime.
"""
...
class GeminiProvider(BaseLLMProvider):
    ...
    async def generate(
        ...
        comp_t = max(25, int(len(last_turn) / 3))
        text = f"[Gemini ({model_id}) Completion] -> Processed: {last_turn[:120]}..."
    ...
    async def stream(
        ...
        last_turn = messages[-1]["content"] if messages else ""
        chunks = [f"[Gemini ({model_id}) ", "Streaming Chunk] ", f"for input: {last_turn[:50]}..."]
        for chunk in chunks:
            await asyncio.sleep(0.005)
            yield chunk
